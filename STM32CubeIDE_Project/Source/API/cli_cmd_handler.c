...
void CLI_CMD_Handler_Set (const char *params) {
    if (params == NULL) {
        DEBUG_API_Error("NULL parameters provided\r");
        return;
    }
    sLedBasicCommandParams_t *cmd_params = (sLedBasicCommandParams_t *)Memory_API_Malloc(sizeof(sLedBasicCommandParams_t));
    if (cmd_params == NULL) {
        DEBUG_API_Error("Memory allocation failed for SET command\r");
        return;
    }
    if (sscanf(params, "%u", &cmd_params->led_number) != 1) {
        DEBUG_API_Error("Invalid parameters provided\r");
        Memory_API_Free(cmd_params);
        return;
    }
    if (cmd_params->led_number > HANDLER_API_MAX_LED_NUMBER) {
        DEBUG_API_Error("Invalid LED number (%u)\r", cmd_params->led_number);
        Memory_API_Free(cmd_params);
        return;
    }
    DEBUG_API_Info("SET command prepared for LED %u\r", cmd_params->led_number);
    Memory_API_Free(cmd_params);
}
...
void CLI_CMD_Handler_Reset (const char *params) {
    ...
    sLedBasicCommandParams_t *cmd_params = (sLedBasicCommandParams_t *)Memory_API_Malloc(sizeof(sLedBasicCommandParams_t));
    if (cmd_params == NULL) {
        DEBUG_API_Error("Memory allocation failed for RESET command\r");
        return;
    }
    if (sscanf(params, "%u", &cmd_params->led_number) != 1) {
        DEBUG_API_Error("Invalid parameters provided\r");
    ...
    DEBUG_API_Info("RESET command prepared for LED %u\r", cmd_params->led_number);
    Memory_API_Free(cmd_params);
}
...
void CLI_CMD_Handler_Toggle (const char *params) {
    ...
    sLedBasicCommandParams_t *cmd_params = (sLedBasicCommandParams_t *)Memory_API_Malloc(sizeof(sLedBasicCommandParams_t));
    if (cmd_params == NULL) {
        DEBUG_API_Error("Memory allocation failed for TOGGLE command\r");
        return;
    }
    if (sscanf(params, "%u", &cmd_params->led_number) != 1) {
        DEBUG_API_Error("Invalid parameters provided\r");
    ...
    DEBUG_API_Info("TOGGLE command prepared for LED %u\r", cmd_params->led_number);
    Memory_API_Free(cmd_params);
}
...
void CLI_CMD_Handler_Blink (const char *params) {
    ...
    if (cmd_params == NULL) {
        DEBUG_API_Error("Memory allocation failed for BLINK command\r");
        return;
    }
    ...
    if ((cmd_params->led_number > HANDLER_API_MAX_LED_NUMBER) || (cmd_params->time > HANDLER_API_MAX_TIME) || (cmd_params->frequency < HANDLER_API_MIN_BLINK_FREQ) || (cmd_params->frequency > HANDLER_API_MAX_BLINK_FREQ)) {
        DEBUG_API_Error("Invalid parameters: LED %u, time %u, frequency %u\r", cmd_params->led_number, cmd_params->time, cmd_params->frequency);
        Memory_API_Free(cmd_params);
        return;
    }
    DEBUG_API_Info("BLINK command prepared for LED %u with time %u and frequency %u\r", cmd_params->led_number, cmd_params->time, cmd_params->frequency);
    Memory_API_Free(cmd_params);
}
